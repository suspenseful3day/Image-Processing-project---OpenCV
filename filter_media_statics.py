...
def create_mediapipe_detector(model_path,
                              min_confidence=0.5):
...
def filter_halo(frame: np.ndarray, bboxes) -> np.ndarray:
    ...
    for bbox in bboxes:
        ...
        if resized_halo is not None:
            ...
            halo_y = y - int(halo_h * 0.75) + y_off
...
def filter_mosaic(frame: np.ndarray, bboxes) -> np.ndarray:
    """모자이크(블러) 필터: 경계선을 아주 부드럽게 그라데이션 처리한 버전"""
    result = frame.copy()
    ...
    for bbox in bboxes:
        ...
        # ── 2. [핵심] 타원 마스크 경계  feather(부드러움) 값 조절 ──
        mask = np.zeros((h, w), dtype=np.uint8)
        cv2.ellipse(mask, (w // 2, h // 2), (w // 2, h // 2), 0, 0, 360, 255, -1)
        
        blur_softness = max(int(w * 0.5), 31) 
        if blur_softness % 2 == 0:
            blur_softness += 1
            
        mask = cv2.GaussianBlur(mask, (blur_softness, blur_softness), 0)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                latency=(detect_end-detect_start)*1000

                self.detect_times.append(latency)

                if len(self.detect_times)>300:
                    self.detect_times.pop(0)

                self.total_detect+=1

                if len(bboxes)>0:
                    self.detect_success+=1
...
def main():
    ...
    while True:
        ...
        if elapsed>=measure_time:
            ...
            avg_latency,success_rate=face_worker.get_stats()

            print("\n=========30초 결과=========")

            print(
                f"FPS : {statistics.mean(fps_list):.2f}"
            )

            print(
                f"CPU : {statistics.mean(cpu_list):.2f}%"
            )

            print(
                f"Memory : {statistics.mean(memory_list):.2f} MB"
            )
